...
public class PlayerControllerActive : MonoBehaviour
{
    ...
    private void Hover()
    {
        ...
        //bodyPassive.AddForce(Vector3.up * hoverForce * hoverForceMultiplier);
    }
    ...
    private void SyncJointsWithIK()
    {
        foreach(ikHolder holder in fingers)
        {
            ...
            while(currentParent != null && currentActiveParent != null)
            {
                ...
                {
                    Vector3 targetVel = Vector3.Cross(currentActiveParent.up, currentParent.up) + Vector3.Cross(currentActiveParent.right, currentParent.right);
                    joint.SetTargetRotationLocal(currentParent.localRotation, cacheRot);
                    //joint.targetAngularVelocity = Vector3.Lerp(joint.targetAngularVelocity, targetVel * 10f, Time.fixedDeltaTime * 4f);
                }
                else
                    break;
